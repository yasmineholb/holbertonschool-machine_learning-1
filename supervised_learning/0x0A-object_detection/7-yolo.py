...
class Yolo:
    ...
    def process_outputs(self, outputs, image_size):
        ...
        box_confidences: the objectness confidences
        box_class_probabilities: probability of each class in box
        """
    ...
    def preprocess_images(self, images):
        ...
        Return
        ndarray of processed images (num images, height, width, 3)
        images shapes (num images, 2)
        """
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        """
        Draw boxes on an image for object predictions.
        """
        image = image.copy()
        ...
        key = cv2.waitKey(0)
        if (key == 115):
            cv2.imwrite('detections/' + os.path.split(file_name)[1], image)
